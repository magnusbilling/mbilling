...
class SearchTariff {
    ...
    public function find( $destination, $id_plan, $id_user, $agi = NULL) {
        ...
        $sql = "SELECT length(prefix) as prefix FROM pkg_prefix WHERE prefix LIKE '".substr( $destination, 0, 2 )."%' ORDER BY length(prefix) DESC LIMIT 1";
        $agi->verbose( $sql, 25 );
        ...
        $sql = "SELECT rateinitial, initblock, billingblock FROM pkg_user_rate WHERE id_prefix = ". $result[0]['id_prefix'] ." AND id_user = ". $id_user;
