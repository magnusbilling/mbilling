...
class Controller extends BaseController
{
    ...
    public function actionRead($asJson = true)
    {
        ...
        $filter = $this->applyFilterToLimitedAdmin($filter);

        if (Yii::app()->session['isAdmin'] == true && isset($_GET['log'])) {
            echo $sql = "SELECT $this->select FROM  " . $this->abstractModel->tableName() . " t $this->join WHERE $filter GROUP BY $group LIMIT $limit";
            echo '<pre>';
            try {
    ...
    {
        if (!in_array(Yii::app()->controller->id, $this->nofilterPerAdminGroup) && Yii::app()->session['user_type'] == 1 && Yii::app()->session['adminLimitUsers'] == true) {

            if (!preg_match("/pkg_user/", $this->join)) {
            ...
            $filter .= " AND $filterByGroup IN (SELECT id_group FROM pkg_group_user_group WHERE id_group_user = " . Yii::app()->session['id_group'] . " )";
        }
        return $filter;
    }
    ...
    public function saveRelated($id, $subRecords, $isUpdate)
    {
        ...
        if ($this->success && is_array($subRecords)) {
            foreach ($subRecords as $item) {
                Yii::log('field = ' . print_r($item, true), 'info');
    ...
    public function actionSoftphone()
    {
        ...
        if (isset($_GET['l'])) {
            ...
            $result[0]['currency']  = $config['global']['base_currency'];
