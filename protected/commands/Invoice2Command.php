...
class Invoice2Command extends CConsoleCommand 
{
	...
	public function run($args)
	{
		...
		foreach ($userResult as $user)
		{
			...
			$filter = "id_user = ".$user['id']." AND starttime < '$today 00:00:00' AND starttime > '$lastMonth 00:00:00'";
			...
			$title = "Total da fatura: ".$config['global']['base_currency'].' '. number_format($cdrSumResult[0]['sessionbill'] * $taxTotal ,2,',','');
			$subTitle = "Total de chamadas: ".number_format($cdrSumResult[0]['nbcall'],0);
			$subTitle2 = "Total de minutos: ".number_format($cdrSumResult[0]['sessiontime'],0);
			//$subTitle3 = "Tax: ". number_format($cdrSumResult[0]['sessionbill'] * '0.'.$tax ,2). ' %';

			$columns ='[
			{"header":"Data","dataIndex":"starttime"},
			{"header":"'.utf8_encode('Número').'","dataIndex":"calledstation"},
			{"header":"'.utf8_encode('Duração').'","dataIndex":"sessiontime"},
			...
			$report->address 	    = 'Endereço: '.$user['address'];
			...
			if (count($smtpResult) > 0) {
				...
				$message = 'Caro cliente '. $user['firstname'] .' ' .$user['lastname'];
				$message .= "<br> Segue em anexo sua fatura detalhada.<br><br>";


				$to_email = $config['global']['admin_email'];
				...
				ob_start();
				try {
					echo 'tentando enviar email para '. $to_email;
					$mail->Send();
